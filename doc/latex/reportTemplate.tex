...
\paragraph{}
The evolution of the number of persons above a given threshold of BMI is strongly dependant on the threshold given but tend to grow. In order to observe if the SISa model could fit this data with any parameter, we went through a range of parameters, chosen with respect to the one found by Cristakis and al. The SISa model with a maximum correlation corresponds to the one with \beta\]=0 and g=0. Thus, we can conclude revalence of automatic contagion over social contagion. 
...
\paragraph{Additional regression on the US obesity by county against the median
% household income in 2010 File obesity\_confounding.m} \
